...
export class DelegateBase implements IDelegate {
	...
	setIWebRtcPlayerController(iWebRtcPlayerController: IWebRtcPlayerController) {
		...
		// create the connect overlay
		this.overlay = this.returnNewOverlay('videoPlayOverlay', 'clickableState', connectOverlayHtml, connectOverlayEvent);
	}
	...
	/**
	 * Creates an overlay for requesting an instance
	 */
	onInstanceRequesting() {
		...
		let onRequestInstanceHtml = document.createElement('div');
		onRequestInstanceHtml.id = 'messageOverlay';
