...
export class StatsPanel {
	...
	public handleStats(stats: AggregatedStats) {
		...
		// Packets lost
		const packetsLostStat = Object.prototype.hasOwnProperty.call(stats.inboundVideoStats, 'packetsLost') ? numberFormat.format(stats.inboundVideoStats.packetsLost) : 'Chrome only';
		this.addOrUpdateStat("PacketsLostStat", "Packets Lost", packetsLostStat);

		// Bitrate
		this.addOrUpdateStat("BitrateStat", "Bitrate (kbps)", stats.inboundVideoStats.bitrate.toString());

		// Video resolution
		const resStat = Object.prototype.hasOwnProperty.call(stats.inboundVideoStats, "frameWidth") 
		&& stats.inboundVideoStats.frameWidth && Object.prototype.hasOwnProperty.call(stats.inboundVideoStats, "frameHeight") 
		&& stats.inboundVideoStats.frameHeight ? stats.inboundVideoStats.frameWidth + 'x' + stats.inboundVideoStats.frameHeight : 'Chrome only';
		this.addOrUpdateStat("VideoResStat", "Video resolution", resStat);

		// Frames decoded
		const framesDecoded = Object.prototype.hasOwnProperty.call(stats.inboundVideoStats, 'framesDecoded') ? numberFormat.format(stats.inboundVideoStats.framesDecoded) : 'Chrome only';
		this.addOrUpdateStat("FramesDecodedStat", "Frames Decoded", framesDecoded);

		// Framerate
		this.addOrUpdateStat("FramerateStat", "Framerate", stats.inboundVideoStats.framerate.toString());

		// Frames dropped
		this.addOrUpdateStat("FramesDroppedStat", "Frames dropped", stats.inboundVideoStats.framesDropped.toString())

		// RTT
		const netRTT = Object.prototype.hasOwnProperty.call(stats.candidatePair, "currentRoundTripTime") 
		&& stats.isNumber(stats.candidatePair.currentRoundTripTime) ? numberFormat.format(stats.candidatePair.currentRoundTripTime * 1000) : 'Can\'t calculate';
		...
		Logger.Log(Logger.GetStackTrace(), `--------- Stats ---------\n ${stats}\n------------------------`, 6);
	}
