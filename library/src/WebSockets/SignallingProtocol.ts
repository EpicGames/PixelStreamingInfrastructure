...
export class SignallingProtocol {
	...
	static setupDefaultHandlers(websocketController : WebSocketController) {
		...
		// PING
		websocketController.signallingProtocol.addMessageHandler(MessageRecvTypes.PING, (pingPayload : string) => {
		...
		// CONFIG
		websocketController.signallingProtocol.addMessageHandler(MessageRecvTypes.CONFIG, (configPayload : string) => {
			// send our pong payload back to the signalling server
		...
		// PLAYER_COUNT
		websocketController.signallingProtocol.addMessageHandler(MessageRecvTypes.PLAYER_COUNT, (playerCountPayload : string) => {
			// send our pong payload back to the signalling server
			Logger.Log(Logger.GetStackTrace(), MessageRecvTypes.PLAYER_COUNT, 6);
			const playerCount: MessagePlayerCount = JSON.parse(playerCountPayload);
			Logger.Log(Logger.GetStackTrace(), "Player Count: " + playerCount.count, 6);
		});

		// ANSWER
		websocketController.signallingProtocol.addMessageHandler(MessageRecvTypes.ANSWER, (answerPayload : string) => {
			// send our pong payload back to the signalling server
		...
		// ICE CANDIDATE
		websocketController.signallingProtocol.addMessageHandler(MessageRecvTypes.ICE_CANDIDATE, (iceCandidatePayload : string) => {
			// send our pong payload back to the signalling server
			Logger.Log(Logger.GetStackTrace(), MessageRecvTypes.ICE_CANDIDATE, 6);
			const iceCandidate: MessageIceCandidate = JSON.parse(iceCandidatePayload);
