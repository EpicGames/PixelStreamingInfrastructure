...
// Browsers disagree on what movementX/movementY mean under pointer lock. Firefox up to 153 warps
// the cursor back to the lock anchor after every move, so it reports the offset from that anchor.
// Firefox 155 stopped doing that and reports the offset from the previous position, which is what
// Chromium has always done. Hardcoding either model makes the test fail on the other one.
//
// Going out to anchor+movement and then straight back to the anchor sidesteps the disagreement:
// the outbound move reports the movement under both models, because under one the reference is the
// anchor and under the other the previous position *is* the anchor. Only the return move differs
// (-movement vs zero), and toContainActions matches a subsequence, so it gets skipped either way.
async function driveLockedMovements(
    page: Page,
    anchor: TranslatedCoordUnsigned,
    movements: { x: number, y: number }[],
    coordConverter: InputCoordTranslator,
    expectedActions: DataChannelEvent[]
) {
...
test('Test locked mouse movement', {
    ...
    const anchor: TranslatedCoordUnsigned = { x: Math.trunc(playerBox!.x + playerBox!.width / 2), y: Math.trunc(playerBox!.y + playerBox!.height / 2) };

    // click on stream to activate pointer lock
    await page.mouse.click(anchor.x, anchor.y);

    // Engaging pointer lock warps the cursor to a browser-defined spot, so the first move
    // afterwards reports a delta we can't predict. Burn it here, outside the capture window,
    // which leaves the mouse at the anchor for the movements below.
    await helpers.settleLockedPointer(page, anchor);

    const events = await getEventSetFrom(streamerPage, async () => {
        await driveLockedMovements(page, anchor, movements, coordConverter, expectedActions);
    });
...
test('Test mouse input after resizing. locked mouse.', {
    ...
    const anchor: TranslatedCoordUnsigned = { x: playerBox!.x + playerBox!.width / 2, y: playerBox!.y + playerBox!.height / 2 };

    // click on stream to activate pointer lock
    await page.click("#streamingVideo");

    // see the note in 'Test locked mouse movement'
    await helpers.settleLockedPointer(page, anchor);
