...
When creating a frontend implementation, these options are visible via the [`Config`](https://github.com/EpicGames/PixelStreamingInfrastructure/Frontend/library/src/Config/Config.ts) object required in order to create a [`PixelStreaming`](https://github.com/EpicGames/PixelStreamingInfrastructure/Frontend/library/src/PixelStreaming/PixelStreaming.ts) stream for your frontend application. Simply set the values you want before initializing the stream object.
