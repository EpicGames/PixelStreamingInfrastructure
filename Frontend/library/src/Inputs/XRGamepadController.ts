    ...
    updateStatus(source: XRInputSource, frame: XRFrame, refSpace: XRReferenceSpace) {
		if (source.gamepad) {
			...
			if(this.controllers[handedness] === undefined) {
				...
				this.controllers[handedness].prevState = WebXRUtils.deepCopyGamepad(source.gamepad);
			}

			this.controllers[handedness].currentState = WebXRUtils.deepCopyGamepad(source.gamepad);
