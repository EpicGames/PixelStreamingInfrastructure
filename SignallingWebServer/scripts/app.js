...
class TwoWayMap {
	constructor() {
		...
		for (const [key, value] in this.map.entries()) {
	...
	remove(key, value) {
		...
		delete this.reverseMap.delete(value);
	}
}
...
// Message is a two way map as when receiving a message we need to look up the message
// via id instead of name.
let messages = new TwoWayMap();
let transceivers = new Map();

function registerMessageHandlers() {
	/*
	 * Recv Only
	 */
	createNewTransceiverAndBindRecv("QualityControlOwnership", onQualityControlOwnership);
	createNewTransceiverAndBindRecv("InitialSettings", onInitialSettings);
	createNewTransceiverAndBindRecv("Response", onResponse);
	createNewTransceiverAndBindRecv("FreezeFrame", onFreezeFrameMessage);
	createNewTransceiverAndBindRecv("UnfreezeFrame", invalidateFreezeFrameOverlay);
	createNewTransceiverAndBindRecv("VideoEncoderAvgQP", onVideoEncoderAvgQP);
	createNewTransceiverAndBindRecv("FileExtension", onFileExtension);
	createNewTransceiverAndBindRecv("FileMimeType", onFileMimeType);
	createNewTransceiverAndBindRecv("FileContents", onFileContents);
	createNewTransceiverAndBindRecv("InputControlOwnership", onInputControlOwnership);
	createNewTransceiverAndBindRecv("Protocol", onProtocolMessage);

	/*
	 * Send Only
	 */
	createNewTransceiverAndBindSend("IFrameRequest", (...args) => sendMessageToStreamer("IFrameRequest", ...args));
	createNewTransceiverAndBindSend("RequestQualityControl", (...args) => sendMessageToStreamer("RequestQualityControl", ...args));
	createNewTransceiverAndBindSend("FpsRequest", (...args) => sendMessageToStreamer("FpsRequest", ...args));
	createNewTransceiverAndBindSend("AverageBitrateRequest", (...args) => sendMessageToStreamer("AverageBitrateRequest", ...args));
	createNewTransceiverAndBindSend("StartStreaming", (...args) => sendMessageToStreamer("StartStreaming", ...args));
	createNewTransceiverAndBindSend("StopStreaming", (...args) => sendMessageToStreamer("StopStreaming", ...args));

	createNewTransceiverAndBindSend("RequestInitialSettings", (...args) => sendMessageToStreamer("RequestInitialSettings", ...args));
	createNewTransceiverAndBindSend("UIInteraction", (...args) => emitUIInteraction(...args));
	createNewTransceiverAndBindSend("KeyDown", (...args) => sendMessageToStreamer("KeyDown", ...args));
	createNewTransceiverAndBindSend("KeyUp", (...args) => sendMessageToStreamer("KeyUp", ...args));
	createNewTransceiverAndBindSend("KeyPress", (...args) => sendMessageToStreamer("KeyPress", ...args));
	createNewTransceiverAndBindSend("MouseEnter", (...args) => sendMessageToStreamer("MouseEnter", ...args));
	createNewTransceiverAndBindSend("MouseLeave", (...args) => sendMessageToStreamer("MouseLeave", ...args));
	createNewTransceiverAndBindSend("MouseDown", (...args) => sendMessageToStreamer("MouseDown", ...args));
	createNewTransceiverAndBindSend("MouseUp", (...args) => sendMessageToStreamer("MouseUp", ...args));
	createNewTransceiverAndBindSend("MouseMove", (...args) => sendMessageToStreamer("MouseMove", ...args));
	createNewTransceiverAndBindSend("MouseWheel", (...args) => sendMessageToStreamer("MouseWheel", ...args));
	createNewTransceiverAndBindSend("MouseDouble", (...args) => sendMessageToStreamer("MouseDouble", ...args));
	createNewTransceiverAndBindSend("TouchStart", (...args) => sendMessageToStreamer("TouchStart", ...args));
	createNewTransceiverAndBindSend("TouchEnd", (...args) => sendMessageToStreamer("TouchEnd", ...args));
	createNewTransceiverAndBindSend("TouchMove", (...args) => sendMessageToStreamer("TouchMove", ...args));
	createNewTransceiverAndBindSend("GamepadButtonPressed", (...args) => sendMessageToStreamer("GamepadButtonPressed", ...args));
	createNewTransceiverAndBindSend("GamepadButtonReleased", (...args) => sendMessageToStreamer("GamepadButtonReleased", ...args));
	createNewTransceiverAndBindSend("GamepadAnalog", (...args) => sendMessageToStreamer("GamepadAnalog", ...args));

	/*
	 * Send and Recv
	 */
	createNewTransceiverAndBindSendRecv("LatencyTest", (...args) => sendMessageToStreamer("LatencyTest", ...args), onLatencyTestMessage);
	createNewTransceiverAndBindSendRecv("Command", emitCommand, onCommand);
}
...
class Transceiver {
	...
	send(...args) {
		if (!this.sender_) {
			console.error("Tried to send a message, but no sender bound")
		}
		this.sender_(args);
	}
	...
	receive(...args) {
		if (!this.receiver_) {
			console.error("Tried to receive a message, but no receiver bound")
		}
		this.receiver_(args);
	}
...
function createNewTransceiverAndBindSendRecv(name, sender, receiver) {
	let transceiver = new Transceiver();
	transceiver.bindSender(sender ? (...args) => sender(...args) : undefined);
	transceiver.bindReceiver(receiver ? (...args) => receiver(...args) : undefined);
	transceivers[name] = transceiver;
}

function createNewTransceiverAndBindSend(name, sender) {
	let transceiver = new Transceiver();
	transceiver.bindSender(sender ? (...args) => sender(...args) : undefined);
	transceivers[name] = transceiver;
}

function createNewTransceiverAndBindRecv(name, receiver) {
	let transceiver = new Transceiver();
	transceiver.bindReceiver(receiver ? (...args) => receiver(...args) : undefined);
	transceivers[name] = transceiver;
}
...
function onVideoEncoderAvgQP(data) {
	VideoEncoderQP = new TextDecoder("utf-16").decode(data[0].slice(1));
}

function onLatencyTestMessage(data) {
	let latencyTimingsAsString = new TextDecoder("utf-16").decode(data[0].slice(1));
...
function onInitialSettings(data) {
	let settingsString = new TextDecoder("utf-16").decode(data[0].slice(1));
...
function onProtocolMessage(data) {
	try {
		let protocolString = new TextDecoder("utf-16").decode(data[0].slice(1));
		let protocolJSON = JSON.parse(protocolString);

		if (protocolJSON.hasOwnProperty('Direction')) {
			console.warn('Received a transmission protocol with a "Direction" field. NOTE: There were breaking changes in how input is handled after UE5.1.x. Please make sure your application version is 5.2 or greater')
			return;
		}

		console.log(`Received new protocol. Updating existing protocol...`);
		Object.keys(protocolJSON).forEach((messageType) => {
			let message = protocolJSON[messageType];
			// Check that the message contains all the relevant params
			if (!message.hasOwnProperty("id")) {
				console.error(`${messageType} message definition was malformed as it didn't contain at least an id\n
							   Definition was: ${JSON.stringify(message, null, 2)}`);
				// return in a forEach is equivalent to a continue in a normal for loop
				return;
			}
			if (message.hasOwnProperty("byteLength") && message.byteLength > 0 && !message.hasOwnProperty("structure")) {
				// If we specify a bytelength, will must have a corresponding structure
				console.error(`${messageType} message definition was malformed as it specified a byteLength but no accompanying structure`);
				// return in a forEach is equivalent to a continue in a normal for loop
				return;
			}
...
function updateStatus() {
	...
	for (let j in controllers) {
		...
		for (let i = 0; i < currentState.buttons.length; i++) {
			...
			if (currButton.pressed) {
				...
				if (i == gamepadLayout.LeftTrigger) {
					...
					transceivers["GamepadAnalog"].send(j, 5, currButton.value);
				} else if (i == gamepadLayout.RightTrigger) {
					//                       UEs right analog has a button index of 6
					transceivers["GamepadAnalog"].send(j, 6, currButton.value);
				} else {
					transceivers["GamepadButtonPressed"].send(j, i, prevButton.pressed);
				}
			} else if (!currButton.pressed && prevButton.pressed) {
				// release
				if (i == gamepadLayout.LeftTrigger) {
					//                       UEs left analog has a button index of 5
					transceivers["GamepadAnalog"].send(j, 6, 0);
				} else if (i == gamepadLayout.RightTrigger) {
					//                       UEs right analog has a button index of 6
					transceivers["GamepadAnalog"].send(j, 6, 0);
				} else {
					transceivers["GamepadButtonReleased"].send(j, i);
				}
			}
		}
		...
		for (let i = 0; i < currentState.axes.length; i += 2) {
			...
			transceivers["GamepadAnalog"].send(j, i + 1, x); // Horizontal axes, only offset by 1
			transceivers["GamepadAnalog"].send(j, i + 2, y); // Vertical axes, offset by two (1 to match UEs axes convention and then another 1 for the vertical axes)
		}
...
function setupHtmlEvents() {
	...
	if (requestKeyframeButton !== null) {
		requestKeyframeButton.onclick = function (event) {
			transceivers["IFrameRequest"].send();
		};
	}
	...
	if (streamSelector) {
		streamSelector.onchange = function (event) {
			const stream = webRtcPlayerObj.get(streamSelector.value);
...
function processFileContents(view) {
	...
	if (file.data.length === file.size) {
		 ...
		 */
		var received = new Blob(file.data, {
			type: file.mimetype
		});
...
function setupWebRtcPlayer(htmlElement, config) {
	...
	webRtcPlayerObj.onDataChannelMessage = function (data) {
		...
		try {
			if (view[0] == 255) {
				// Protocol is the only hardcoded message type as we need to know before we receive the protocol message
				// what its id is
				transceivers["Protocol"].receive(data);
				return;
			}
			let messageName = messages.getFromId(view[0]);
			transceivers[messageName].receive(data);
		} catch (e) {
			console.error(e);
		}
	};
...
function updateVideoStreamSize() {
	...
	if (now - lastTimeResized > 1000) {
		...
		transceivers["Command"].send(descriptor)
...
function sendMessageToStreamer(messageType, indata = []) {
	messageFormat = messages.getFromName(messageType);
...
function emitDescriptor(messageType, descriptor) {
	while (descriptor.constructor === Array) {
		descriptor = descriptor[0];
	}
	// Convert the descriptor object into a JSON string.
	let descriptorAsString = JSON.stringify(descriptor);
	let messageFormat = messages.getFromName(messageType);
	if (messageFormat === undefined) {
...
function releaseMouseButtons(buttons, x, y) {
	...
	if (buttons & MouseButtonsMask.PrimaryButton) {
		transceviers["MouseUp"].send(MouseButton.MainButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.SecondaryButton) {
		transceviers["MouseUp"].send(MouseButton.SecondaryButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.AuxiliaryButton) {
		transceviers["MouseUp"].send(MouseButton.AuxiliaryButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.FourthButton) {
		transceviers["MouseUp"].send(MouseButton.FourthButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.FifthButton) {
		transceviers["MouseUp"].send(MouseButton.FifthButton, coord.x, coord.y);
	}
}
...
function pressMouseButtons(buttons, x, y) {
	let coord = normalizeAndQuantizeUnsigned(x, y);
	if (buttons & MouseButtonsMask.PrimaryButton) {
		transceivers["MouseDown"].send(MouseButton.MainButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.SecondaryButton) {
		transceivers["MouseDown"].send(MouseButton.SecondaryButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.AuxiliaryButton) {
		transceivers["MouseDown"].send(MouseButton.AuxiliaryButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.FourthButton) {
		transceivers["MouseDown"].send(MouseButton.FourthButton, coord.x, coord.y);
	}
	if (buttons & MouseButtonsMask.FifthButton) {
		transceivers["MouseDown"].send(MouseButton.FifthButton, coord.x, coord.y);
	}
}
...
function registerLockedMouseEvents(playerElement) {
	...
	function lockStateChange() {
		if (document.pointerLockElement === playerElement ||
			...
			[...new Set(activeKeys)].forEach((uniqueKeycode) => {
				transceivers["KeyUp"].send(uniqueKeycode);
			});
	...
	function updatePosition(e) {
		...
		transceivers["MouseMove"].send(coord.x, coord.y, delta.x, delta.y);
	}


	playerElement.onmousedown = function (e) {
		transceivers["MouseDown"].send(e.button, coord.x, coord.y);
	};

	playerElement.onmouseup = function (e) {
		transceivers["MouseUp"].send(e.button, coord.x, coord.y);
	};

	playerElement.onwheel = function (e) {
		transceivers["MouseWheel"].send(e.wheelDelta, coord.x, coord.y);
	};

	playerElement.ondblclick = function (e) {
		transceivers["MouseDouble"].send(e.button, coord.x, coord.y);
	};
...
function registerTouchEvents(playerElement) {
	...
	function emitTouchData(type, touches) {
		for (let t = 0; t < touches.length; t++) {
			...
			switch (type) {
				case "TouchStart":
					transceivers["TouchStart"].send(numTouches, coord.x, coord.y, fingerIds[touch.identifier], MaxByteValue * touch.force, coord.inRange ? 1 : 0);
					break;
				case "TouchEnd":
					transceivers["TouchEnd"].send(numTouches, coord.x, coord.y, fingerIds[touch.identifier], MaxByteValue * touch.force, coord.inRange ? 1 : 0);
					break;
				case "TouchMove":
					transceivers["TouchMove"].send(numTouches, coord.x, coord.y, fingerIds[touch.identifier], MaxByteValue * touch.force, coord.inRange ? 1 : 0);
					break;
			}
		}
	}
		...
		playerElement.ontouchend = function (e) {
			for (let t = 0; t < e.changedTouches.length; t++) {
				...
				if (touch.identifier === finger.id) {
					...
					let coord = normalizeAndQuantizeUnsigned(x, y);
					transceivers["MouseUp"].send(MouseButton.MainButton, coord.x, coord.y);
		...
		playerElement.ontouchmove = function (e) {
			for (let t = 0; t < e.touches.length; t++) {
				...
				if (touch.identifier === finger.id) {
					...
					transceiver["MouseMove"](coord.x, coord.y, delta.x, delta.y);
					finger.x = x;
					finger.y = y;
					break;
				}
...
function registerKeyboardEvents() {
	...
	document.onkeydown = function (e) {
		...
		transceivers["KeyDown"].send(keyCode, e.repeat);
	...
	document.onkeyup = function (e) {
		...
		transceivers["KeyUp"].send(keyCode, e.repeat);
	...
	document.onkeypress = function (e) {
		...
		transceivers["KeyPress"].send(e.charCode);
	};
}
